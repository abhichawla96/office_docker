package com.esign.docusgindirect.util;
...
public class DocuSignUtil {
	...
	private APIServiceSoap getWSClient() throws Exception{
		try {
			...
			logger.debug("Going to make connection to docuesign URL : "
					+ wsdlLocation);
			...
			APIService apiService = new APIService(url, new QName(
			...
			logger.debug("EndPoint from WSDL :"+bp.getRequestContext().get(BindingProvider.ENDPOINT_ADDRESS_PROPERTY));
	...
	public EnvelopeStatus sendDocument(List<DocuSignRequest> docusignRequestList) throws Exception{
		...
		try{
			if(docusignRequestList!=null && docusignRequestList.size()>0){
				...
				String subject="e-Signature";
				try {
					if(ESIGN_MAIL_SUBJECT!=null && !ESIGN_MAIL_SUBJECT.isEmpty())
						subject=ESIGN_MAIL_SUBJECT;
				}
				catch(Exception e)
				{
					subject="e-Signature";
					logger.error("Error in setting subjectline eSign " + e.getMessage());
					e.printStackTrace();
				}
				envolope.setSubject(subject);
	...
	public EnvelopeStatus checkStatusESignDoc(String identifier) throws Exception{
		...
		try {
			...
			//e.printStackTrace();
			logger.error("Unable to execute checkStatusESignDoc method due to error : " + DataUtils.getExceptionStackTrace(e));
			throw e;
		}
	...
	public Envelope retreiveDocument(String identifier) throws Exception{
		...
		try {
			...
			//e.printStackTrace();
			logger.error("Unable to execute retreiveDocument method due to error : " + DataUtils.getExceptionStackTrace(e));
			throw e;
		}
								...
								for(int l=0; l<recipientIdentifierList.size(); l++){
									...
									if(docuSignRequestObj.getDocAnchorInfo().contains(recipientIdentifier)){
										...
										if(anchorString.contains("DATE")){
											...
											if(arrayOfTab != null && arrayOfTab.getTab() != null && arrayOfTab.getTab().size()>0){
												List<Tab> tablListTemp =  arrayOfTab.getTab();
												for(int n=0; n<tablListTemp.size(); n++){
													count =0;
													Tab tabTemp =  tablListTemp.get(n);
													if(tabTemp!=null){
														if(tabTemp.getType().equals(TabTypeCode.DATE_SIGNED)){
															count =1;
															break;
														}
													}
										...
										tab.setDocumentID(new BigInteger(Integer.toString(docuSignRequestObj.getDocumentId())));
										tab.setRecipientID(new BigInteger(Integer.toString(recipientInfoObj.getRecipientId())));
	...
	private ArrayOfRecipient getRecipientList(List<DocuSignRequest> docuSignRequestList) throws Exception {
		...
		for(int i=0;i<docuSignRequestList.size();i++){
			DocuSignRequest docuSignRequest = docuSignRequestList.get(i);
			List<RecipientInfoBean> recipientInfoList =  docuSignRequest.getRecipientInfoList();
			List<RecipientInfoBean> reciepientInfoListTemp = checkForDuplicateRecipient(recipientInfoList,finalRecipientInfoList,docuSignRequest);
			
			/*if(reciepientInfoListTemp != null){
				for(int k=0; k<reciepientInfoListTemp.size(); k++){
	...
	public byte[] getBytesFromFile(String pdfFilePath) throws Exception{
		...
		try {
			...
			//e.printStackTrace();
			logger.error("Unable to execute getBytesFromFile method due to error : " + DataUtils.getExceptionStackTrace(e));
		}
		...
		return bytes;
			...
			//e1.printStackTrace();
			logger.error("Unable to execute getBytesFromFile method due to error : " + DataUtils.getExceptionStackTrace(e1));
			throw e1;
		}
	...
	public String getRequestXml(Envelope requestObject) throws Exception{
		try {
			...
			//e.printStackTrace();
			logger.error("Unable to execute getRequestXml method due to error : " + DataUtils.getExceptionStackTrace(e));
			throw e;
		}
	...
	public String getResponseXml(EnvelopeStatus resObject) throws Exception{
		try {
			...
			//e.printStackTrace();
			logger.error("Unable to execute getResponseXml method due to error : " + DataUtils.getExceptionStackTrace(e));
			throw e;
		}
	...
	private List<RecipientInfoBean> checkForDuplicateRecipient(List<RecipientInfoBean> recipientInfoBeanList, List<RecipientInfoBean> finalRecipientInfoList,DocuSignRequest docuSignRequest){
			...
			if(finalRecipientInfoList != null && finalRecipientInfoList.size()>0){
				...
				for(int i=0;i<recipientInfoBeanList.size();i++){
					...
					for(int k=0; k<finalRecipientInfoList.size(); k++){
						...
						if(emailCompareFrom != null && emailCompareTo !=  null && (emailCompareFrom.equals(emailCompareTo))){
							...
							if(userNameCompareFrom != null && initialsCompareFrom != null && userNameCompareTo != null && initialsCompareTo != null
									&& userNameCompareFrom.equals(userNameCompareTo) && initialsCompareFrom.equals(initialsCompareTo)){
								List<String> recipientIdentifierList = new ArrayList<String>();
								List<String> recipientDocList = new ArrayList<String>();
								List<String> recipientIdentifierListTemp = finalRecipientInfoObj.getRecipientIdentifierList();
								List<String> recipientIdentifierListTemp1 = recipientInfoObj.getRecipientIdentifierList();
